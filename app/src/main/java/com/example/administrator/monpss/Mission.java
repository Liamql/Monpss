 ...
 */
class Mission extends BaseExpandableListAdapter {
    ...
    public Mission(Context context, ArrayList<Entity> groupList)
    {
    ...
    public void updateListView(ArrayList<Entity> list) {
        this.groupList = list;
        ...
        addDone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(misstitle.length()>0) {
                    ...
                    if(par_miss_id >=0)
                    {
                        db.execSQL("insert into Mission(mission_name,parentID,parent_name,level) values(?,?,?,?)",new Object[]{misstitle,par_miss_id,groupList.get(position).mainMission,1});
                    }
